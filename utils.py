...
def addEvent(username, result):
    db = getDB()
    db.update({'username': username}, {'$push' : {'events' : {'location': result[location], 'date' : result[date], 'time' : result[time], 'attendees' : result[attendees]}}})
